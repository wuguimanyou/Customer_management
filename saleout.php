...
<div class="WSY_content" id="WSY_content_height">
    ...
    <?php
		...
		$query_count="select count(1) as tcount FROM weixin_commonshop_products WHERE isvalid=true AND customer_id=".$customer_id." and storenum <= 0 and isout = 0";
	    ...
	    $query2="SELECT id,name,asort_value,type_id,type_ids,orgin_price,now_price,default_imgurl,isnew,createtime,isout,ishot,isnew,good_level,meu_level,bad_level,is_supply_id,create_type,sell_count,is_QR,storenum 
			FROM weixin_commonshop_products WHERE isvalid=true AND customer_id=".$customer_id." and storenum <= 0 and isout = 0"; 
		$query3="";
    ...
    <div class="WSY_data">
	  ...
	  ?>
              ...
              <tr id="WSY_q1">
				...
				<?php if($_SESSION['is_auth_user']=='no' or ($_SESSION['is_auth_user']=='yes' and $p_isout==1)){ // 如果是授权用户,则需要商家下架后才能编辑 或者 商家才能编辑?>
				  <?php if((($owner_general==1 and $create_type==1) or ($owner_general==2 and $create_type==2) or ($owner_general==0 and $create_type==3)) or ($create_type==-1) ){ ?>
				<a href="saleout.php?customer_id=<?php echo $customer_id_en; ?>&keyid=<?php echo $p_id; ?>&op=del" onclick="if(!confirm(&#39;删除后不可恢复，继续吗？&#39;)){return false};" title="删除"><img src="../../../common/images_V6.0/operating_icon/icon04.png"></a>
                 <?php }
			}?>
				</td>
