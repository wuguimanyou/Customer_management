...
$query="SELECT w.id,w.name ,w.weixin_name,w.weixin_headimgurl,w.phone,p.parent_id,p.is_consume,p.isAgent,p.createtime,p.status,w.fromw FROM promoters p INNER JOIN weixin_users w ON w.id=p.user_id  WHERE  w.isvalid=true and p.isvalid=TRUE AND p.status=1  AND match(w.gflag) against (',".$scene_id.",')";
//echo $query;	 
$result_q = mysql_query($query) or die('Query failed: ' . mysql_error());	
$rcount_q = mysql_num_rows($result_q);
$query = $query." order by p.id desc"." limit ".$start.",".$end;

//更新总的推广员数
 $query_update="update promoters set team_prom=".$rcount_q." where isvalid=true  and customer_id=".$customer_id." and user_id=".$scene_id."";
			...
			<tbody>
			   <?php 
	            ...
	            while ($row = mysql_fetch_object($result)) {
					...
					$num=0;
					$query2="select count(*) as num from weixin_commonshop_promoter_changes where isvalid=true and customer_id=".$customer_id." and user_id=".$user_id." and remark='商家后台手动改动关系'";
...
var page = count;
	...
	$(".WSY_page").createPage({
		...
		document.location= "qrsell_detail.php?pagenum="+p+"&customer_id="+customer_id+"&scene_id="+scene_id;
	   }
    });
  ...
  function jumppage(){
	...
	if((a<1) || (a==pagenum) || (a>page) || isNaN(a)){
		...
		document.location= "qrsell_detail.php?pagenum="+a+"&customer_id="+customer_id+"&scene_id="+scene_id;
